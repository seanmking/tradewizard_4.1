...
from .base import BaseModule, ModuleOutput, StandardizedMCPData
from llm_interpreter.llm_client import call_llm # Assuming LLM client is here
from llm_interpreter.taxo_engine import classify_products # Import taxonomy engine
...
class WebsiteAnalysisModule(BaseModule):
    ...
    async def build_payload(self, assessment_data: Dict[str, Any], products: Optional[List[Dict]] = None) -> Dict[str, Any]:
        ...
        # This Module primarily works off the raw_content generated by the crawler.
    ...
    async def run(self, payload: Dict[str, Any]) -> ModuleOutput:
        ...
        # Note: We assume trigger_crawler logic happens *before* this Module runs,
        ...
        if not crawler_data_dict:
            ...
            return ModuleOutput(assessment_id=assessment_id, mcp_name=self.name, status="error", error="Missing crawler_data", result={})
        ...
        try:
            ...
            return ModuleOutput(assessment_id=assessment_id, mcp_name=self.name, status="error", error=f"Invalid crawler_data format: {e}", result={})
        ...
        if crawl_status == "failed":
            logger.error(f"Assessment {assessment_id}: Crawler failed for {url}. Error: {crawl_error}. Halting WebsiteAnalysisModule.")
            ...
            return ModuleOutput(assessment_id=assessment_id, mcp_name=self.name, status="error", error=f"Crawler failed: {crawl_error}", _db_patch=db_patch, result={})
        ...
        # run taxonomy on scraped variants
        clusters = classify_products(extracted_products)
        ...
        if not api_key:
            ...
            return ModuleOutput(assessment_id=assessment_id, mcp_name=self.name, status="error", error="OpenAI API key not configured", _db_patch=db_patch, result={})
        ...
        analysis_warnings = [] # Placeholder for warnings during this Module's analysis (not crawler warnings)
        try:
            ...
            return ModuleOutput(assessment_id=assessment_id, mcp_name=self.name, status="error", error=f"Prompt generation failed: {e}", _db_patch=db_patch, result={}) # Return patch with crawler data even if prompt fails
        ...
        # --- Step 6: Construct Final Module Output --- 
        ...
        # attach scraped products and taxonomy clusters to result
        if llm_response_data is None:
            llm_response_data = {}
        llm_response_data["products"] = extracted_products
        llm_response_data["taxonomy_clusters"] = clusters
        return ModuleOutput(
